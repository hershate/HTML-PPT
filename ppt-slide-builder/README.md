...
1. **初始化项目** — 逐轮提问收集需求（标题/风格/页数/内容）→ 12 维度系统性分析优化项目规约（每项推理链可追溯）→ 严格按照规约从模板生成 HTML 文件
2. **添加幻灯片** — 向用户确认新幻灯片内容和布局后在末尾追加，自动编号
3. **修改幻灯片内容** — 通过 slide 标记精确定位，仅修改内容区域
4. **添加入场动画** — 为指定 slide 中的元素添加 CSS 动画 class
5. **修改页面过渡** — 设置整体过渡类型（淡入、滑动、缩放、翻转等）
6. **修改主题样式** — 用户要求模糊时追问具体风格方向，通过 CSS 变量统一修改
7. **删除幻灯片** — 移除目标 slide 并重新编号所有后续 slide
8. **预览** — 在默认浏览器中打开
...
- 如需占位图片，使用 placeholder 服务并在之后替换
